...
def test_artifact_sql_evidence_is_preloaded_without_per_artifact_get() -> None:
    source = (ROOT / "forza" / "application" / "db_doctor" / "artifact_checks.py").read_text(encoding="utf-8")
    invalid_body = _function_body(source, "_invalid_file_artifacts", "_file_matches_size_and_sha256")
    preload_body = _function_body(source, "_artifact_sql_evidence_keys", "_artifact_has_sql_evidence")
    predicate_body = _function_body(source, "_artifact_has_sql_evidence", "_attempt_has_debug_evidence")

    assert "_artifact_sql_evidence_keys(session, artifacts)" in invalid_body
    assert "session.get(" not in invalid_body
    assert "session.get(" not in preload_body
    assert "session.get(" not in predicate_body
    assert "ExtractionAttemptEntity.id.in_(chunk)" in preload_body
    assert "range(0, len(attempt_ids), 900)" in preload_body


def test_batched_artifact_sql_evidence_preserves_raw_and_failed_attempt_semantics(tmp_path, migrated_db_path: Path) -> None:
    db_path = migrated_db_path
    ids = _seed_valid_run(db_path)
    bad_artifact = tmp_path / "bad-request-preview.json"
        ...
        with Session(engine) as session:
            ...
            session.add(
                ModelArtifactEntity(
                    ...
                    file_path=str(tmp_path / "missing-raw.json"),
                    sha256="0" * 64,
                    size_bytes=999,
                    is_canonical=True,
                )
            ...
            error_image_path = tmp_path / "error.png"
            error_image_path.write_bytes(b"error image")
            error_image = ImageFileEntity(
                id="image-error",
                file_hash=file_hash(error_image_path),
                file_name=error_image_path.name,
                current_name=error_image_path.name,
                current_path=str(error_image_path),
            )
            ...
            error_input = RunInputEntity(
                ...
                input_order=1,
                input_path=error_image.current_path,
                file_name=error_image_path.name,
                extension=".png",
                file_hash=error_image.file_hash,
                decision="process",
            )
            ...
            error_attempt.request_hash = canonical_request_hash(
                ...
                source_file_hash=error_image.file_hash,
            ...
            session.add(
                ModelArtifactEntity(
                    id="sql-backed-failed-attempt",
                    run_id=str(ids["run_id"]),
                    image_file_id=error_image.id,
                    extraction_result_id=error_result.id,
                    attempt_id=error_attempt.id,
                    artifact_type="failed_attempt",
                    file_path=str(tmp_path / "missing-failed.json"),
                    sha256="1" * 64,
                    size_bytes=500,
                    is_canonical=False,
                )
            ...
            session.add(
                ModelArtifactEntity(
                    id="invalid-request-preview",
                    run_id=str(ids["run_id"]),
                    image_file_id=str(ids["image_file_id"]),
                    extraction_result_id=str(ids["result_id"]),
                    attempt_id=str(ids["attempt_id"]),
                    artifact_type="request_preview",
                    file_path=str(bad_artifact),
                    sha256="2" * 64,
                    size_bytes=123,
                    is_canonical=False,
                )
