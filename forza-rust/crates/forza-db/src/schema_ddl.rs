...
pub const TABLE_DDL: &[&str] = &[
    ...
    // export_artifacts
    r#"CREATE TABLE export_artifacts (
	id VARCHAR NOT NULL, 
	run_id VARCHAR, 
    ...
    r#"CREATE TABLE external_lap_records (
	id VARCHAR NOT NULL, 
...
,
    ...
    r#"CREATE TABLE external_record_imports (
	id VARCHAR NOT NULL, 
...
,
    ...
    // extraction_attempts
    r#"CREATE TABLE extraction_attempts (
	id VARCHAR NOT NULL, 
...
,
    ...
    // extraction_results
    r#"CREATE TABLE extraction_results (
	id VARCHAR NOT NULL, 
	run_id VARCHAR NOT NULL, 
...
,
    ...
    // extraction_runs
    r#"CREATE TABLE extraction_runs (
	id VARCHAR NOT NULL, 
...
,
    ...
    // model_artifacts
    r#"CREATE TABLE model_artifacts (
	id VARCHAR NOT NULL, 
	run_id VARCHAR NOT NULL, 
    ...
    r#"CREATE TABLE model_runtime_snapshots (
	id VARCHAR NOT NULL, 
	run_id VARCHAR NOT NULL, 
    ...
    // prompt_snapshots
    r#"CREATE TABLE prompt_snapshots (
	id VARCHAR NOT NULL, 
    ...
    // reference_tracks
    r#"CREATE TABLE reference_tracks (
	id VARCHAR NOT NULL, 
	name VARCHAR NOT NULL, 
	normalized_name VARCHAR, 
	aliases_json JSON, 
...
,
    ...
    // review_corrections
    r#"CREATE TABLE review_corrections (
            id VARCHAR NOT NULL,
...
pub const INDEX_DDL: &[&str] = &[
    ...
    r#"CREATE UNIQUE INDEX idx_attempts_one_accepted_per_result ON extraction_attempts (extraction_result_id) WHERE accepted = 1"#,
    // idx_runtime_one_preflight_per_run
    r#"CREATE UNIQUE INDEX idx_runtime_one_preflight_per_run ON model_runtime_snapshots (run_id) WHERE snapshot_kind = 'preflight'"#,
    // idx_attempts_reason
    r#"CREATE INDEX idx_attempts_reason ON extraction_attempts (attempt_reason)"#,
    // idx_attempts_run_status
    r#"CREATE INDEX idx_attempts_run_status ON extraction_attempts (run_id, status)"#,
    // idx_external_lap_records_active_order
    r#"CREATE INDEX idx_external_lap_records_active_order ON external_lap_records (track, race_class, best_lap_ms) WHERE active = 1"#,
    // idx_extraction_results_image_file
    r#"CREATE INDEX idx_extraction_results_image_file ON extraction_results (image_file_id)"#,
    // idx_extraction_results_status
    r#"CREATE INDEX idx_extraction_results_status ON extraction_results (status)"#,
    // idx_extraction_runs_created
    r#"CREATE INDEX idx_extraction_runs_created ON extraction_runs (created_at)"#,
    // idx_extraction_runs_status
    r#"CREATE INDEX idx_extraction_runs_status ON extraction_runs (status)"#,
    // idx_image_files_best_lap_status
    r#"CREATE INDEX idx_image_files_best_lap_status ON image_files (best_lap_status)"#,
    // idx_image_files_duplicate_of
    r#"CREATE INDEX idx_image_files_duplicate_of ON image_files (duplicate_of_image_file_id)"#,
    // idx_image_files_file_modified_at
    r#"CREATE INDEX idx_image_files_file_modified_at ON image_files (file_modified_at)"#,
    // idx_image_files_hash
    r#"CREATE INDEX idx_image_files_hash ON image_files (file_hash)"#,
    // idx_image_files_status
    r#"CREATE INDEX idx_image_files_status ON image_files (file_status)"#,
    // idx_image_flags_image_file_type_status
    r#"CREATE INDEX idx_image_flags_image_file_type_status ON image_flags (image_file_id, flag_type, status)"#,
    // idx_lap_records_best_gui_order
    r#"CREATE INDEX idx_lap_records_best_gui_order ON lap_records (track, race_class, weather, best_lap_ms, driver, car) WHERE is_best_lap = 1"#,
    // idx_lap_records_best_track_class_car
    r#"CREATE INDEX idx_lap_records_best_track_class_car ON lap_records (track_normalized, race_class, car_normalized) WHERE is_best_lap = 1"#,
    // idx_lap_records_best_track_class_driver
    r#"CREATE INDEX idx_lap_records_best_track_class_driver ON lap_records (track_normalized, race_class, driver_normalized) WHERE is_best_lap = 1"#,
    // idx_lap_records_image_file
    r#"CREATE INDEX idx_lap_records_image_file ON lap_records (image_file_id)"#,
    // idx_lap_records_track_class_car
    r#"CREATE INDEX idx_lap_records_track_class_car ON lap_records (track_normalized, race_class, car_normalized)"#,
    // idx_lap_records_track_class_driver
    r#"CREATE INDEX idx_lap_records_track_class_driver ON lap_records (track_normalized, race_class, driver_normalized)"#,
    // idx_model_artifacts_attempt
    r#"CREATE INDEX idx_model_artifacts_attempt ON model_artifacts (attempt_id)"#,
    // idx_model_artifacts_run_image_file
    r#"CREATE INDEX idx_model_artifacts_run_image_file ON model_artifacts (run_id, image_file_id)"#,
    // idx_model_runtime_run_kind
    r#"CREATE INDEX idx_model_runtime_run_kind ON model_runtime_snapshots (run_id, snapshot_kind)"#,
    // idx_review_cases_outcome
    r#"CREATE INDEX idx_review_cases_outcome ON review_cases (outcome)"#,
    // idx_review_cases_reason_trigger
    r#"CREATE INDEX idx_review_cases_reason_trigger ON review_cases (reason, "trigger")"#,
    // idx_review_cases_status_reason
    r#"CREATE INDEX idx_review_cases_status_reason ON review_cases (status, reason)"#,
    // idx_review_corrections_image_file_field
    r#"CREATE INDEX idx_review_corrections_image_file_field ON review_corrections (image_file_id, field)"#,
    // idx_run_inputs_hash
    r#"CREATE INDEX idx_run_inputs_hash ON run_inputs (file_hash)"#,
    // idx_run_inputs_image_file
    r#"CREATE INDEX idx_run_inputs_image_file ON run_inputs (image_file_id)"#,
    // idx_run_inputs_process_reason
